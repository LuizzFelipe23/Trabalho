    ...
    public static void salvarChefs(String nomeArquivo, List<Chefe> chefs) {
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(nomeArquivo))) {
            for (Chefe chefe : chefs) {
                writer.write(chefe.toString()); // Utiliza o método toString() para formatar os dados
    ...
    public static void carregarChefs(String nomeArquivo, List<Chefe> chefs) {
        ...
        try {
            ...
            while ((linha = reader.readLine()) != null) {
                String[] dados = linha.split("\\|"); // Utiliza "\\|" para separar os dados
                if (dados.length == 5) {
                    int id = Integer.parseInt(dados[0].trim().substring(4)); // Remove "ID: " e converte para int
                    String nome = dados[1].trim().substring(7); // Remove "Nome: "
                    int idade = Integer.parseInt(dados[2].trim().substring(7)); // Remove "Idade: " e converte para int
                    String cpf = dados[3].trim().substring(6); // Remove "CPF: "
                    String cargo = dados[4].trim().substring(8); // Remove "Cargo: "
